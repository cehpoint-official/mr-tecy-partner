...
export function LocationDialog({ open, onOpenChange, onSave, initialLocation }: LocationDialogProps) {
    ...
    // Update state when initialLocation changes or dialog opens
    useEffect(() => {
        if (open && initialLocation) {
            setSelectedLocation(initialLocation);
            fetchDistrictName(initialLocation.lat, initialLocation.lng);
        } else if (open && typeof window !== "undefined") {
            // Fallback to local storage if no initial location provided
    ...
    const fetchDistrictName = async (lat: number, lng: number) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Internal API returns the Nominatim data directly
